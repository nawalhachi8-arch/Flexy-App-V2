...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Use a try-catch block to handle potential errors if window.Telegram is not available
    try {
        ...
        if (!telegramUser || !telegramUser.id) {
            setError("لا يمكن التعرف على حساب تيليجرام الخاص بك.");
            setIsLoading(false);
            return;
        }
        ...
        const fetchUserData = async () => {
          try {
            ...
            setError("حدث خطأ أثناء تحميل بياناتك من السيرفر. تأكد أنك بدأت قاعدة البيانات في وضع الاختبار.");
          } finally {
            setIsLoading(false);
          }
        };
  ...
  const handleWatchAd = async () => {
    ...
    try {
        ...
        setPoints(points); // Revert optimistic update on error
  ...
  const handleWithdrawalSubmit = async (e: React.FormEvent) => {
    ...
    // Securely fetch points from Firestore before submitting
    ...
    try {
      ...
      if (data.ok) {
        ...
        // Reset points in Firestore
        const userDocRef = doc(db, "users", userId);
        const newPointsAfterWithdrawal = 0;
