...
const SpinnerWheel = ({ onSpin, isSpinning, result }: { onSpin: () => void; isSpinning: boolean; result: number | null }) => {
  ...
  // This function calculates the final rotation angle for the wheel
  const getRotationAngle = () => {
    if (result === null) return 0;
    // Find the index of the winning segment
    const resultIndex = segments.indexOf(result);
    // Base angle to point the indicator at the start of the winning segment
    const baseAngle = 360 - (resultIndex * segmentAngle);
    // Add a random offset to make it land somewhere within the segment, not always at the edge
  ...
  const rotationStyle = {
    // Apply the rotation transform. It animates over 6 seconds.
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    (window as any).onAdClosedCallback = onAdClosedCallback;
  ...
  const triggerAd = () => {
    ...
    console.log("Attempting to show video ad...");
    showToast("جاري تحميل الإعلان...");
    // This is a placeholder for showing a real ad.
    // The onAdClosedCallback should be called by the ad network's SDK.
    setTimeout(() => {
      onAdClosedCallback();
    }, 2000); // Simulating an ad view
  };
  ...
  return (
    <>
      <button 
        ...
        className={`fixed bottom-28 right-6 z-40 w-14 h-14 rounded-full shadow-lg flex items-center justify-center text-white transition-all ${spinsLeft > 0 ? 'bg-primary animate-pulse' : 'bg-muted'}`}
      ...
      <div dir="rtl">
        ...
        <div 
            ...
            className="fixed bottom-0 left-0 w-full h-16 bg-gray-200 flex items-center justify-center text-sm text-gray-500 z-30"
        >
            <p>منطقة إعلان البانر</p>
        </div>
      </div>
    </>
  );
