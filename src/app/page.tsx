...
const SpinnerWheel = ({ onSpin, isSpinning, resultIndex }: { onSpin: () => void; isSpinning: boolean; resultIndex: number | null }) => {
    const segments = [
        { value: 500, color: "#FF6347" },
        { value: 100, color: "#FFD700" },
        { value: 400, color: "#1E90FF" },
        { value: 200, color: "#BA55D3" },
        { value: 300, color: "#ADFF2F" },
    ];
    const segmentAngle = 360 / segments.length;

    const getRotationAngle = () => {
        if (resultIndex === null) return 0;
        const winningAngle = resultIndex * segmentAngle;
        const randomOffset = (Math.random() - 0.5) * (segmentAngle * 0.8);
        const targetAngle = winningAngle + (segmentAngle / 2) + randomOffset;
        return 360 * 5 + (360 - targetAngle);
    };

    const rotationStyle = {
        transform: `rotate(${isSpinning && resultIndex !== null ? getRotationAngle() : 0}deg)`,
    ...
    const getTextCoordinates = (angle: number, distance: number) => {
      const radians = (angle - 90) * (Math.PI / 180);
                    ...
                    <svg viewBox="0 0 220 220" className="w-full h-full">
                        <g>
                            {segments.map((segment, index) => {
                                const midAngle = (segmentAngle * index) + (segmentAngle / 2);
                                const { x, y } = getTextCoordinates(midAngle + 3, radius * 0.65);
                                return (
                                    <g key={index}>
                                        <path d={getPathForSegment(index)} fill={segment.color} />
                                        <text
                                            x={x}
                                            y={y}
                                            fill="white"
                        ...
                        <circle cx={center} cy={center} r={radius} fill="none" stroke="hsl(var(--primary))" strokeWidth="8" />
                        <circle cx={center} cy={center} r="8" fill="white" stroke="hsl(var(--primary))" strokeWidth="3" />
                    </svg>
                </div>
                {/* Pointer */}
...
export default function Home() {
  ...
  const handleSpin = async () => {
    ...
    const startSpin = () => {
      ...
      setSpinResultIndex(winningIndex);
      setIsSpinning(true);
      ...
      setTimeout(async () => {
          ...
          setIsSpinning(false);
          setSpinResultIndex(null); // Reset for next spin
      }, 6500); // Match animation duration + a small buffer
    };
  ...
  const handleWithdrawalSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        💰 Points to Withdraw: ${MIN_WITHDRAWAL_POINTS.toLocaleString()}
        💰 User's Total Points: ${currentPointsOnServer.toLocaleString()}
        🆔 User ID: ${userId}
        ---------------------------------
        `;
        ...
        {showSpinnerDialog && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
            <div className="card sm:max-w-md w-full relative">
              ...
              <div className="p-6">
               {spinsLeft > 0 ? (
                 <SpinnerWheel onSpin={handleSpin} isSpinning={isSpinning} resultIndex={spinResultIndex} />
               ) : (
